...
from ebsdsim.engine.integrate import PerVoltageContext
...
def test_prescan_dispatch():
    ...
    pg = build_pg_k_grid(cell.pg_num, hw=2, space_group=cell.space_group)
...
def test_lambert_fill_gpu_matches_cpu():
    ...
    ctx = require_gpu()
    hw = 2
    pg = build_pg_k_grid(cell.pg_num, hw, space_group=cell.space_group)
    n_k = pg.kij.size // 3
