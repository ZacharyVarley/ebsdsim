...
from ebsdsim.save import cell_metadata
...
_FIXTURES = Path(__file__).resolve().parent / "data" / "cif"
...
def test_dual_origin_227_hall_ops_disagree_with_sg_op_data():
    """CIF-path Hall ops (origin 2) disagree with legacy SG_OP_DATA for Fd-3m."""
    pos = (0.125, 0.125, 0.125)
    hall_orbit = expand_orbit_with_ops(ops_from_hall(227), pos)
    legacy_orbit = expand_sg_orbit(227, pos)
    assert len(hall_orbit) == 8
    assert len(legacy_orbit) != len(hall_orbit)
    assert ops_from_hall(227).shape == (192, 12)


def test_dual_origin_142_hall_ops_disagree_with_sg_op_data():
    """SG 142 is dual-origin; Hall (origin 2) orbits differ from SG_OP_DATA."""
    pos = (0.0, 0.0, 0.0)
    hall_n = len(expand_orbit_with_ops(ops_from_hall(142), pos))
    legacy_n = len(expand_sg_orbit(142, pos))
    assert hall_n != legacy_n
