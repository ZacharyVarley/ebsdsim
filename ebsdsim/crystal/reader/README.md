...
```python
from ebsdsim.crystal.reader import hall_ops, STD_HALL
...
| `verify.py` | `python -m ebsdsim.crystal.reader.verify` |
...
```bash
python -m ebsdsim.crystal.reader.setting --rebuild
...
Manual `Material` construction uses the same Hall-operator orbit expansion
as CIF ingest.
