...
from ebsdsim.crystal.elements import DEFAULT_B_ISO_ANGSTROM_SQ
from ebsdsim.crystal.simcell import ANGSTROM_SQ_TO_NM_SQ
...
# Zero U_iso/B_iso in a CIF is treated as missing: unphysical at finite T and
# drives pathological n_strong in the WK absorption / beam-selection path.
DEFAULT_B_ISO_NM_SQ = DEFAULT_B_ISO_ANGSTROM_SQ * ANGSTROM_SQ_TO_NM_SQ
DEFAULT_U_ISO_ANGSTROM_SQ = DEFAULT_B_ISO_ANGSTROM_SQ / (8.0 * np.pi ** 2)
...
def _parse_cell_atoms(blk):
    ...
    if uiso:
        ...
        u = np.zeros(n)
        thermal_tags = False
    return cell, species, coords, occ, u, thermal_tags


def _sanitize_uiso(u, *, thermal_tags):
    """Replace missing/non-positive Uiso with the room-temperature default.

    When ``_atom_site_U_iso_or_equiv`` / ``_atom_site_B_iso_or_equiv`` are present
    but non-positive, warn that explicit zero is unphysical. When the tags are
    absent, still apply the default but do not claim a zero was found in the file.
    Callers should snapshot as-read CIF sites *before* this.
    """
    u = np.asarray(u, dtype=float).reshape(-1).copy()
    bad = ~np.isfinite(u) | (u <= 0.0)
    if not np.any(bad):
        return u
    n_bad = int(np.count_nonzero(bad))
    u[bad] = DEFAULT_U_ISO_ANGSTROM_SQ
    if thermal_tags:
        print(
            "[ebsdsim] CIF has non-positive (0) U_iso/B_iso on "
            "%d/%d site(s); zero Debye-Waller is unphysical at finite temperature. "
            "Using default B_iso=%g A^2 (%g nm^2) for those sites."
            % (n_bad, u.size, DEFAULT_B_ISO_ANGSTROM_SQ, DEFAULT_B_ISO_NM_SQ),
            flush=True,
        )
    else:
        print(
            "[ebsdsim] CIF has no U_iso/B_iso on %d/%d site(s); "
            "using default B_iso=%g A^2 (%g nm^2) for those sites."
            % (n_bad, u.size, DEFAULT_B_ISO_ANGSTROM_SQ, DEFAULT_B_ISO_NM_SQ),
            flush=True,
        )
...
def _standard_metric_penalty(n, cell):
    ...
    sysname = crystal_system(n)
    ang = lambda x, t: abs(x - t)  # noqa: E731 — compact metric helpers
    eq = lambda x, y: abs(x - y)  # noqa: E731
    if sysname == "monoclinic":
...
def structure_from_block(blk, source="", eps=DEFAULT_EPS):
    ...
    cell, species, coords, occ, u, thermal_tags = _parse_cell_atoms(blk)
    ...
    # Keep cif_input.uiso as authored; sanitize the working array used for sim.
    u = _sanitize_uiso(u, thermal_tags=thermal_tags)
