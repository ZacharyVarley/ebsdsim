...
class Setting:
    ...
    def describe(self):
        # Fraction only used for human-readable origin-shift text.
        from fractions import Fraction  # kept local: rare debug path
...
def _metric_ok(sysname, cell):
    ...
    """
    a, b, c, al, be, ga = cell
    eq = lambda x, y: abs(x - y) < 1e-4  # noqa: E731 — compact metric helpers
    ang90 = lambda x: abs(x - 90.0) < _ATOL  # noqa: E731
...
def _settings_table():
    ...
    Regenerate with ``python -m ebsdsim.crystal.reader.setting --rebuild``.
...
def metric_family(cell):
    ...
    """
    a, b, c, al, be, ga = (float(v) for v in cell)
    eq = lambda x, y: abs(x - y) < _ATOL * max(1.0, abs(y))  # noqa: E731 — compact metric helpers
    ang90 = lambda x: abs(x - 90.0) < _ATOL  # noqa: E731
    ang120 = lambda x: abs(x - 120.0) < _ATOL  # noqa: E731
...
if __name__ == "__main__":
    import sys  # CLI rebuild entrypoint only
    import time
...
def _derived_positions(ops):
    # Lazy: verify is optional / CLI-oriented and imports setting.
