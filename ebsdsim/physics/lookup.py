"""Physics layer: structure-factor lookup tables and coupling scales."""
...
from ebsdsim.physics.scattering import scatter_factor_wk_array
...
def reciprocal_length(h: Vec3, cell: SimCell) -> float:
    return float(np.sqrt(max(_metric_dot(h, cell.reciprocal_metric), 0.0)))


def reciprocal_length_squared(h: Vec3, cell: SimCell) -> float:
...
def _first_below_dmin(axis: int, cell: SimCell, dmin: float) -> int:
...
def hkl_limits(cell: SimCell, dmin: float) -> tuple[int, int, int]:
    return (
...
def is_reflection_allowed(h: int, k: int, l: int, cell: SimCell) -> bool:  # noqa: E741 — Miller l
...
def generate_reflections(cell: SimCell, dmin: float, difference_table: bool) -> NDArray[np.int32]:
    ...
    for h in range(-hr, hr + 1):
        for k in range(-kr, kr + 1):
            for l in range(-lr, lr + 1):  # noqa: E741 — Miller l
...
def _stack_site_positions(cell: SimCell) -> tuple[NDArray[np.float64], NDArray[np.bool_]]:
...
def prepare_diff_lookup_geometry(cell: SimCell, dmin: float) -> DiffLookupGeometry:
...
def build_diff_lookup(cell: SimCell, opts: BuildLookupOptions) -> DiffLookupData:
