"""GPU layer: Lambert FS→square rasterization."""
...
from ebsdsim.energy.weights import reduce_over_sites
...
from ebsdsim.lambert.rasterize import build_rasterize_pixel_maps
...
class GpuLambertRasterizer:
    ...
    def __init__(
        ...
        *,
        ...
        self._wgsl = load_wgsl("lambert/lambert_fill.wgsl")
        ...
        # One view fills 2 sheets (NH + SH) of plane_size f32. The GPU never holds
        # every energy bin at once: each dispatch fills a bounded band of views and
        # the host concatenates, so large halfw × many bins stays under the binding.
        binding = int(
            device_limit(self.device, "max_storage_buffer_binding_size", 128 * 1024 * 1024)
        )
        self._band_views = max(1, int(binding // (2 * self.plane_size * 4)))

    def _ensure_capacity(self, view_count: int) -> None:
        cap = min(view_count, self._band_views)
        if cap <= self._capacity_views and self._sheet_buffer and self._out_buffer:
            return
        if self._sheet_buffer is not None:
            self._sheet_buffer.destroy()
        if self._out_buffer is not None:
            self._out_buffer.destroy()
        byte_length = max(4, cap * 2 * self.plane_size * 4)
    ...
    def expand_sheets(
        ...
        *,
        ...
        stacked = np.asarray(stacked_sheets, dtype=np.float32).reshape(-1)
        view_stride = 2 * self.plane_size
        out = (
            np.empty(view_count * view_stride, dtype=np.float32)
            if readback
            else None
        )
        band = 0
        while band < view_count:
            n_band = min(self._band_views, view_count - band)
            self._ensure_capacity(n_band)
            assert self._sheet_buffer is not None
            assert self._out_buffer is not None
            src = band * view_stride
            self._sheet_buffer.write(stacked[src : src + n_band * view_stride])
            self._dispatch(n_band)
            if out is not None:
                out[src : src + n_band * view_stride] = self._out_buffer.read_as(
                    np.float32, size=n_band * view_stride * 4
                )
            band += n_band
        if out is None:
            return np.zeros(0, dtype=np.float32)
