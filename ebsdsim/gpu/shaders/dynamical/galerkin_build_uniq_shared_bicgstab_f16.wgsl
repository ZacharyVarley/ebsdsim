/*
 * Galerkin rational Krylov (RKSM) solve — BiCGSTAB Smith columns + Galerkin epilogue.
 *
 * Builds the same rational Krylov subspace as the Smith path: the Cayley
 * transform T = (qI + A)(qI − A)⁻¹ converts the continuous Lyapunov equation
 *   A X + X Aᴴ + e₀e₀ᴴ = 0
 * into a Stein equation, and BiCGSTAB column applications of T produce the
 * columns Z = [B̃, T B̃, T² B̃, …] with B̃ = √(2q)(qI − A)⁻¹ e₀ — a rational
 * Krylov subspace of A with a single repeated pole at q.
 *
 * The Smith path combines those columns with truncated geometric-series
 * coefficients. This Galerkin path (RKSM in the Druskin–Simoncini sense)
 * instead orthonormalizes them (modified Gram–Schmidt), forms the projected
 * pencil H = Qᴴ A Q and b = Qᴴ e₀ (raw e₀, not √(2q)·e₀; A = qI − Q_minus
 * unscaled — Jacobi scl used by BiCGSTAB does not enter), then a follow-on
 * dispatch solves the small m×m Lyapunov equation on-device and expand
 * forms W = Q F. Optimal over the subspace → same accuracy at lower rank.
 *
 * Same LLL pack ladder as smith: resident / unique-Δ / tile. Krylov is
 * BiCGSTAB on A = S Q- S (Hermitian products) for odd U.
 *
 * stats bit28 = unique-Δ, bit29 = BiCGSTAB, bit30 = unique-seg TILE,
 * bit31 = dense TILE, bits 20–24 = kept (MGS column count), 0xFFFFFFFF = fail.
 * Host string-replaces MAX_N / MAX_PACK / MAX_UNIQ / BPT for beam buckets.
 *
 * Bindings (group 0):
 *   0  params          1  hkl           2  table
 *   3  idx_a           4  d_a           5  e0
 *   6  q_values        7  w_stack       8  stats
 *   9  uniq_bits      10  uniq_vals    11  uniq_meta
 *  14  galerkin_h  (r×r c64 per batch, params.rank = h_ld stride)
 *  15  galerkin_b  (r   c64 per batch, params.rank = h_ld stride)
 * Bindings 12/13 are reserved for adaptive-intensity sgh_tables / sgh_pack.
 *
 * 0.2.2 matvec fix: self-interaction subtracted once after the inner j-loop.
 * Private `my_i` array: WGSL forbids dynamic indexing of private arrays.
 */
...
// Krylov / Galerkin column cap (g_rdiag length; host params.rank ≤ this).
const MAX_RANK: u32 = 16u;
// galerkin_h / galerkin_b leading dim = params.rank (host h_ld / krylovRank).
...
// Galerkin projection buffers (skip 12/13 — adaptive intensity).
@group(0) @binding(14) var<storage, read_write> galerkin_h: array<vec2<f32>>; // h_ld² per batch
@group(0) @binding(15) var<storage, read_write> galerkin_b: array<vec2<f32>>; // h_ld  per batch
...
var<workgroup> g_rdiag: array<f32, MAX_RANK>; // |R_jj| from Gram-Schmidt, for column drop
var<workgroup> g_kept: u32;
...
fn main(
    ...
    // Broadcast any_fail uniformly before the epilogue if/else (both sides use
    // barriers). Private any_fail is set from reduction-driven col_ok; Tint can
    // treat it as non-uniform without a workgroupUniformLoad fence.
    if (lid == 0u) { g_kept = any_fail; } // reuse g_kept as scratch before MGS
    workgroupBarrier();
    let fail_u = workgroupUniformLoad(&g_kept);

    // ------------------------------------------------------------------
    // Galerkin epilogue: MGS over w_stack, then H = Qᴴ A Q, b = Qᴴ e0.
    // A = q I - Q_minus (unscaled; no Jacobi scl). Raw e0, not scale·e0.
    // ------------------------------------------------------------------
    var kept = 0u;
    if (fail_u == 0u) {
        for (var j = 0u; j < params.rank; j = j + 1u) {
            var qc: array<vec2<f32>, BPT>;
            for (var t = 0u; t < BPT; t = t + 1u) {
                let live = (lid + t * WG) < n;
                qc[t] = select(vec2<f32>(0.0, 0.0),
                               w_stack[wbase + orig[t] * params.rank + j], live);
            }
            for (var i = 0u; i < kept; i = i + 1u) {
                var acc = vec2<f32>(0.0, 0.0);
                for (var t = 0u; t < BPT; t = t + 1u) {
                    let live = (lid + t * WG) < n;
                    let qi = select(vec2<f32>(0.0, 0.0),
                                    w_stack[wbase + orig[t] * params.rank + i], live);
                    acc = cx_add(acc, cx_mul(cx_conj(qi), qc[t]));
                }
                let d = reduce_sum_c(lid, acc);
                for (var t = 0u; t < BPT; t = t + 1u) {
                    let live = (lid + t * WG) < n;
                    let qi = select(vec2<f32>(0.0, 0.0),
                                    w_stack[wbase + orig[t] * params.rank + i], live);
                    qc[t] = cx_sub(qc[t], cx_mul(d, qi));
                }
            }
            var nl = 0.0;
            for (var t = 0u; t < BPT; t = t + 1u) {
                let live = (lid + t * WG) < n;
                if (live) { nl = nl + cx_abs2(qc[t]); }
            }
            let nrm = sqrt(reduce_sum_f(lid, nl));
            if (lid == 0u) { g_rdiag[j] = nrm; }
            workgroupBarrier();
            // Tint treats bare workgroup reads as non-uniform; uniform-load R_00
            // so the keep branch (and kept++) stays uniform for later barriers.
            let r00 = workgroupUniformLoad(&g_rdiag[0]);
            // Drop near-dependent columns (the ones ADI stalls on).
            if (nrm > 1.0e-6 * r00) {
                let inv = 1.0 / nrm;
                for (var t = 0u; t < BPT; t = t + 1u) {
                    let live = (lid + t * WG) < n;
                    if (live) {
                        w_stack[wbase + orig[t] * params.rank + kept] = cx_scale(qc[t], inv);
                    }
                }
                kept = kept + 1u;
            }
        ...
        if (lid == 0u) { g_kept = kept; }
        workgroupBarrier();
        kept = workgroupUniformLoad(&g_kept);

        // Zero H / b slots for this batch (stride = params.rank = host h_ld).
        let h_ld = max(params.rank, 1u);
        let hbase = batch * h_ld * h_ld;
        let bbase = batch * h_ld;
        for (var ii = lid; ii < h_ld * h_ld; ii = ii + WG) {
            galerkin_h[hbase + ii] = vec2<f32>(0.0, 0.0);
        }
        for (var ii = lid; ii < h_ld; ii = ii + WG) {
            galerkin_b[bbase + ii] = vec2<f32>(0.0, 0.0);
        }
        workgroupBarrier();

        // H[:, j] = Qᴴ (A q_j), A = q I - Q_minus (unscaled gather).
        for (var j = 0u; j < kept; j = j + 1u) {
            var qj: array<vec2<f32>, BPT>;
            for (var t = 0u; t < BPT; t = t + 1u) {
                let live = (lid + t * WG) < n;
                qj[t] = select(vec2<f32>(0.0, 0.0),
                               w_stack[wbase + orig[t] * params.rank + j], live);
                if (live) { sp[my_i[t]] = qj[t]; } // no Jacobi scl
            }
            workgroupBarrier();
            ...
            if (mode == 0u) {
                for (var t = 0u; t < BPT; t = t + 1u) {
                    ...
                    for (var jj = 0u; jj < n; jj = jj + 1u) {
                        let idx = u32(si - ss[jj] + off);
                        if (idx < plen) {
                            acc = cx_add(acc, cx_mul(load_u(idx), sp[jj]));
                        }
                    }
                    acc = cx_sub(acc, cx_mul(load_u(u32(off)), sp[i]));
                    let diag = cx_sub(vec2<f32>(q1, 0.0), Dd[t]);
                    let qpv = cx_sub(cx_mul(diag, sp[i]), acc);
                    Apd[t] = cx_sub(cx_scale(qj[t], q), qpv); // A qj, no scl
                }
            } else if (mode == 1u) {
                for (var t = 0u; t < BPT; t = t + 1u) {
                    let live = (lid + t * WG) < n;
                    ...
                    for (var jj = 0u; jj < n; jj = jj + 1u) {
                        let idx = u32(si - ss[jj] + off);
                        acc = cx_add(acc, cx_mul(load_u(unique_rank(idx)), sp[jj]));
                    }
                    acc = cx_sub(acc, cx_mul(load_u(unique_rank(u32(off))), sp[i]));
                    let diag = cx_sub(vec2<f32>(q1, 0.0), Dd[t]);
                    let qpv = cx_sub(cx_mul(diag, sp[i]), acc);
                    Apd[t] = cx_sub(cx_scale(qj[t], q), qpv);
                }
            } else if (mode == 3u) {
                var seg0 = 0u;
                let vbase = batch * MAX_NU_CAP;
                loop {
                    ...
                    for (var t = 0u; t < BPT; t = t + 1u) {
                        ...
                        for (var jj = 0u; jj < n; jj = jj + 1u) {
                            ...
                            let rank = unique_rank_g(batch, idx);
                            if (rank >= seg0 && (rank - seg0) < seglen) {
                                acc = cx_add(acc, cx_mul(load_u(rank - seg0), sp[jj]));
                            }
                        }
                        let self_rank = unique_rank_g(batch, u32(off));
                        if (self_rank >= seg0 && (self_rank - seg0) < seglen) {
                ...
                loop {
                    ...
                    for (var t = 0u; t < BPT; t = t + 1u) {
                        ...
                        for (var jj = 0u; jj < n; jj = jj + 1u) {
                            let idx = u32(si - ss[jj] + off);
                            if (idx >= tile0 && (idx - tile0) < tlen) {
                                acc = cx_add(acc, cx_mul(load_u(idx - tile0), sp[jj]));
                            }
                        }
                        let self_off = u32(off) - tile0;
                        if (u32(off) >= tile0 && self_off < tlen) {
            ...
            // H[i, j] = <q_i, A q_j>
            // reduce_sum_c returns via workgroupUniformLoad, so every lane has
            // hij. Lane i writes the scalar (no extra barrier); staging into
            // shared then bursting ≤16 stores would add a barrier that costs
            // more than it saves at this size.
            for (var i = 0u; i < kept; i = i + 1u) {
                var hip = vec2<f32>(0.0, 0.0);
                for (var t = 0u; t < BPT; t = t + 1u) {
                    let live = (lid + t * WG) < n;
                    let qi = select(vec2<f32>(0.0, 0.0),
                                    w_stack[wbase + orig[t] * params.rank + i], live);
                    hip = cx_add(hip, cx_mul(cx_conj(qi), Apd[t]));
                }
                let hij = reduce_sum_c(lid, hip);
                if (lid == i) {
                    galerkin_h[hbase + i * h_ld + j] = hij;
                }
            }
            workgroupBarrier();
        }
        ...
        // b[i] = <q_i, e0>  (raw e0)
        for (var i = 0u; i < kept; i = i + 1u) {
            var bip = vec2<f32>(0.0, 0.0);
            for (var t = 0u; t < BPT; t = t + 1u) {
                let live = (lid + t * WG) < n;
                if (!live) { continue; }
                let qi = w_stack[wbase + orig[t] * params.rank + i];
                bip = cx_add(bip, cx_mul(cx_conj(qi), e0[n_base + orig[t]]));
            }
            let bi = reduce_sum_c(lid, bip);
            if (lid == i) {
                galerkin_b[bbase + i] = bi;
            }
        }
        workgroupBarrier();
    } else {
        // Failed batch: leave H/b zeroed for this slot.
        let h_ld = max(params.rank, 1u);
        let hbase = batch * h_ld * h_ld;
        let bbase = batch * h_ld;
        for (var ii = lid; ii < h_ld * h_ld; ii = ii + WG) {
    ...
    if (lid == 0u) {
        if (any_fail != 0u) {
            ...
            // bits 20–24: kept MGS columns (0..16)
            st = st | ((kept & 0x1Fu) << 20u);
            stats[batch] = st;
        }
        if (mode == 1u || mode == 3u) {
